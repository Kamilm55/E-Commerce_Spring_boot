...
//    }
    ...
    @GetMapping("/{email}")
    public ResponseEntity<LoggedInUserDetailsDTO> getUserDetails(@PathVariable("email") String email){
        return ResponseEntity.ok(loggedInUserDetailsService.getUserDetailsByEmail(email));
    }

    @PutMapping("/{email}")
    public ResponseEntity<LoggedInUserDetailsDTO> updateUser(@PathVariable("email") String email,@RequestBody UpdateUserDetailsPayload updateUserDetailsPayload ){
    ...
    //todo: check status code for best practice
}
